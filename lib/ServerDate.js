...
var
  ...
  // synchronization if no url is specified.
  ...
  config = {
    ...
    debug: true,
    // Recommended value of samples that we have a good chance of at least one sample with low latency.
    samples: 10,
    // Dummy url for synchronization. If no specified ServerDate will be returned
    url: scriptUrl,
...
function mergeOptions() {
  var defaults = window.ServerDateConfig;
  if (defaults) {
...
function synchronize() {
  ...
  // Ask the server for another empty response or copy of ServerDate.js but specify a unique number on the url querystring
	...
	request.open("GET", config.url + "?noCache=" + Date.now());
  ...
  function processSample(serverNow) {
    ...
    // Take the specified samples (default 10) so we get a good chance of at least one sample with
    // low latency.
    if (iteration < config.samples) {
      iteration++;
      requestSample();
    }
    else {
